...
# Bases para comparar
prod_base['prod_total'] = prod_total
prod_base['prod_term'] = prod_term
...
for label,(d,prod_key,base) in deltas_g.items():
    ...
    pbase = prod_base[prod_key]
    pnew = new_prod[prod_key]

    if pbase <=0 or pnew<=0:
        continue

    costo_original = base/pbase
    costo_nuevo_comp = (base+d)/pnew

    imp = costo_nuevo_comp-costo_original
...
for pk,idxs in componentes_por_prod.items():
    ...
    for idx in idxs:
        ...
        gasto_total = costo_unitario*pbase

        nuevo_costo = gasto_total/pnew

        imp = nuevo_costo-costo_unitario
