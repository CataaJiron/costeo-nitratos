...
with st.sidebar:
    ...
    st.divider()
    pagina = st.radio("", ["Dashboard", "Analisis mensual", "Sensibilidad PPTO", "Sensibilidad R+P",  "Gastos por Área","Sim. Gastos PPTO","Sim. Gastos R+P"],
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    import copy
    st.title("Simulador de Sensibilidad — Real + Proyección (Apertura de Gastos)")
 
    col_v, _ = st.columns([2, 6])
    with col_v:
        modo_sens = st.radio("Vista", ["Puntual", "Acumulado"], horizontal=True,
                             label_visibility="collapsed", key="modo_sg_rp")
    tipo_sens = "Puntual" if modo_sens == "Puntual" else "Acumulado"
    mes = botones_mes("sg_rp")
    st.divider()
    ...
    # ── helpers R+P: busca REAL primero, si no hay busca PROY ────────────────
    def _r(area, subarea, concepto, medida=None, nth=0):
        for t2 in ['REAL', 'PROY']:
            mask = ((df['Fecha']==fecha)&(df['AREA']==area)&(df['SUBAREA']==subarea)&
                    (df['CONCEPTO']==concepto)&(df['Tipo']==tipo_sens)&(df['Tipo_2']==t2))
            if medida: mask = mask & (df['Medida']==medida)
            r = df[mask]['GASTO/COSTO']
            if len(r) > nth:
                return float(r.values[nth])
    ...
    # ── Mostrar qué tipo de dato se está usando ──────────────────────────────
    tiene_real = not df[(df['Fecha']==fecha)&(df['Tipo_2']=='REAL')].empty
    tipo_dato  = "REAL" if tiene_real else "PROY"
    st.caption(f"📊 Usando datos: **{tipo_dato}** para {MESES[mes]}")
    ...
    if ('rp_sg_sv' not in st.session_state or
            st.session_state.get('rp_sg_mes') != mes or
            st.session_state.get('rp_sg_tipo') != tipo_sens):
        st.session_state['rp_sg_sv']   = copy.deepcopy(BASE)
        st.session_state['rp_sg_base'] = copy.deepcopy(BASE)
        st.session_state['rp_sg_mes']  = mes
        st.session_state['rp_sg_tipo'] = tipo_sens
        st.session_state['rp_sg_rc']   = st.session_state.get('rp_sg_rc', 0) + 1
    if 'rp_sg_rc' not in st.session_state:
        st.session_state['rp_sg_rc'] = 0
    rp_rc = st.session_state['rp_sg_rc']
    V     = st.session_state['rp_sg_sv']
    BASE  = st.session_state['rp_sg_base']
    for k, val in BASE.items():
        if k not in V: V[k] = val
 
    # ── recalcular — idéntico a Sim. Gastos PPTO ─────────────────────────────
    def recalcular(v):
        npt3       = v['KNO3_T_NPT3'] + v['KNO3_R_NPT3']
        npt4       = v['KNO3_L_NPT4'] + v['CSSI_NPT4']  + v['CSSR_NPT4']
        prod_total = npt3 + npt4
        ...
        B_PNV = sum(BASE[k] for k in ['PNV_REMUN','PNV_ENERG','PNV_ARRDO','PNV_OTROS','PNV_MANT_D','PNV_MANT_M','PNV_PRECO_A','PNV_PRECO_O','PNV_PRODU_A','PNV_PRODU_O'])
        B_PPB = sum(BASE[k] for k in ['PPB_REMUN','PPB_MYREP','PPB_COMB','PPB_ARRDO','PPB_OTROS','PPB_DIST_EE','PPB_MANT_D','PPB_MANT_M','PPB_PRECO_A','PPB_PRECO_O','PPB_PRODU_A','PPB_PRODU_O'])
        B_PCS = sum(BASE[k] for k in ['PCS_REMUN','PCS_MYREP','PCS_ENERG','PCS_ARRDO','PCS_AGUA','PCS_OTROS','PCS_MANT_D','PCS_MANT_M','PCS_PRECO_A','PCS_PRECO_O','PCS_PRODU_A','PCS_PRODU_O'])
        g_nv_fin = BASE['G_POZAS_NV'] + (g_pnv - B_PNV)
        g_pb_fin = BASE['G_POZAS_PB'] + (g_ppb - B_PPB)
        g_cs_fin = BASE['G_POZAS_CS'] + (g_pcs - B_PCS)
        c12 = (g_nv_fin+g_pb_fin+g_cs_fin+v['G_DEPRECIACION_CS'])/prod_total if prod_total>0 else 0.0
        ...
        B_N3 = sum(BASE[k] for k in ['N3_REMUN','N3_ENERG','N3_PETROL','N3_MAQ','N3_AGUA','N3_MYREP','N3_ARRDO','N3_CSODA','N3_OTROS','N3_MANT_D','N3_MANT_M','N3_KORDA'])
        B_N4 = sum(BASE[k] for k in ['N4_REMUN','N4_ENERG','N4_PETROL','N4_MAQ','N4_AGUA','N4_CSODA','N4_OTROS','N4_MANT_D','N4_MANT_M','N4_KORDA'])
        g_n3_fin = BASE['G_NPT3'] + (g_n3 - B_N3)
        g_n4_fin = BASE['G_NPT4'] + (g_n4 - B_N4)
        c13 = (g_n3_fin+g_n4_fin+v['DEP_NPT3']+v['DEP_NPT4'])/prod_total if prod_total>0 else 0.0
    ...
    with col_inp:
        ...
        def ni(label, key, val, step=10.0, fmt="%.1f"):
            return st.number_input(label, value=float(val), step=step, format=fmt, key=f"rp_{key}_{rp_rc}")
    ...
    with col_res:
        ...
        dd = 0.0 if abs(delta_total) < 0.005 else round(delta_total, 2)
        ds = "0.00 USD/T" if dd == 0.0 else f"{dd:+.2f} USD/T"
        st.markdown(f"#### 📊 Resultado — {MESES[mes]} ({tipo_dato})")
        st.metric("R+P Base",  f"${costo_base:.2f} / T")
        st.metric("Simulado",  f"${costo_sim:.2f} / T", delta=ds, delta_color="inverse")
        st.divider()
        rows_r=[{"Componente":k,"R+P":round(b,2),"Sim":round(s,2),"Δ":round(s-b,2)}
                for k,(b,s) in zip(comp_base.keys(),zip(comp_base.values(),comp_sim.values()))]
        df_res=pd.DataFrame(rows_r)
        def _cd(val):
            if isinstance(val,float):
                ...
                if val<0: return 'color:#80BC00;font-weight:bold'
            return ''
        st.dataframe(df_res.style.map(_cd,subset=["Δ"]).format({"R+P":"{:.2f}","Sim":"{:.2f}","Δ":"{:+.2f}"}),
                     use_container_width=True,hide_index=True,height=360)
        st.divider()
        if st.button("🔄 Restablecer valores R+P", use_container_width=True, key="rp_sg_reset"):
            st.session_state['rp_sg_rc']   = st.session_state.get('rp_sg_rc',0)+1
            st.session_state['rp_sg_sv']   = copy.deepcopy(st.session_state['rp_sg_base'])
            st.session_state['rp_sg_mes']  = mes
            st.session_state['rp_sg_tipo'] = tipo_sens
            st.rerun()